...
class TestOrder:
    ...
    def test_order_scooter(self, firefox_driver, button_type, first_name, last_name, delivery_address, metro_station, phone, delivery_date, rental_period, color, comment):  # Проверка на успешное создание заказа
        ...
        self.order_page = OrderPage(firefox_driver)
        self.confirm_order_page = ConfirmOrderPage(firefox_driver)
        self.home_page.click_order_button(button_type)
        self.order_page.fill_personal_data(first_name, last_name, delivery_address, metro_station, phone)
