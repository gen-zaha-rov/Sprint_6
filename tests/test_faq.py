...
class TestFAQ:
    ...
    def test_faq_answers(self, firefox_driver, question_index, expected_answer):  # Проверить, что после нажатия на вопрос откроется текст ответа.
        ...
        BasePage.scroll_down_to_bottom()  # Прокрутить в самый низ страницы

        self.home_page.click_faq_question(question_index)
        actual_answer = self.home_page.get_faq_answer_text(question_index)
